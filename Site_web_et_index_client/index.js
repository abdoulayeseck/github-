...
async function requestGet(url) {
  ...
  const response = requestGet(state.url+'/logement');
  response.then(filterHttpResponse)
      .then((finalData) => {
   //       console.log(finalData);
            finalData.forEach((element) => {
                console.log(element.nom_region);
                let balise_liste_region = document.querySelector("#list_region");
                html = `
                                <span>    
                                    ${element.nom_region}
                                </span>    
                ...
                div.innerHTML = html;
                div.className='row';
                balise_liste_region.append(div);
            });
      });
...
EXEMPLE DE REQUETE POST
  const response = requestPost(state.url+'/link', {'data1': '2876'});
  response.then(filterHttpResponse)
      .then((finalData) => {
          console.log(finalData);
      });
...
fetchdata().then(datapoints =>{
    ...
    myChart.config.data.labels = region;
    myChart.config.data.datasets[0].data = rehabilitation_prixderevient_logement;
    myChart.update();
});
};
...
// render init block
const data = {
      labels: ['Redbull', 'Blue', 'Yellow', 'Green', 'Purple', 'Orange'],
      datasets: [{
        label: '# of Votes',
        data: [12, 19, 3, 5, 2, 3],
        borderWidth: 1
